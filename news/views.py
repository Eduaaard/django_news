...
def login_view(requests):
    if requests.method == "POST":
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(requests, user)
                messages.success(requests, "Вы успешно вошли в аккаунт")
    ...
    return render(requests, "pages/login.html", context)


def registration_view(requests):
    if requests.method == "POST":
        form = RegistrationForm(data=requests.POST)
        if form.is_valid():
            form.save()
            messages.success(requests, "Пользователь добавлен")
    ...
    return render(requests, "pages/registration.html", context)


def user_logout(requests):
    logout(requests)
    messages.error(requests, "Вы успешно вышли из аккаунта", extra_tags="danger")
    return redirect("home")


def add_article(requests):
    if requests.method == "POST":
        form = ArticleForm(data=requests.POST, files=requests.FILES)
        if form.is_valid():
            form = form.save(commit=False)
            form.author = requests.user
            form.save()
            messages.success(requests, "Статья успешно была созданна ТВАРЬ!")
    ...
    return render(requests, "pages/article_form.html", context)
